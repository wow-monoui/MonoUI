...
mCTi:SetScript("OnEvent", function(self, event, ...)
    local subEvent = ...;
    if event == "COMBAT_TEXT_UPDATE" then
		local data, arg3, arg4 = GetCurrentCombatTextEventInfo();
        info = tbl[subEvent]
        if (subEvent=="HEAL" or subEvent=="HEAL_CRIT" or subEvent=="PERIODIC_HEAL") and arg3 < cfg.combattext.threshold.heal then return end
        if (subEvent=="HONOR_GAINED") and abs(data)<1 then return end
        if (info) then
            local msg = info.prefix or ""
            if (info.spec) then
                if (arg3) then
                    msg = template:format(data, arg3)
                end
            else
                if (info.data) then
					msg = msg .. floor(data)
...
function mCTd:OnEvent(event, ...)
    if cfg.combattext.show_damage then mCTd:ProcessDamageEvent(event, ...) end
end
...
function mCTh:OnEvent(event, ...)
    if cfg.combattext.show_healing then mCTh:ProcessHealingEvent(event, ...) end
end

-- Outgoing damage
function mCTd:ProcessDamageEvent(event, ...)
...
-- Outgoing healing
function mCTh:ProcessHealingEvent(event, ...)
    local unpack, select, time = unpack, select, time
    local msg, icon
    local timestamp, eventType, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags = ...
