...
-- wrapper around UnitBuff after changes to WoW API
lib.UnitBuff = function(unit, id)
	for i = 1, 40 do
...
-- wrapper around UnitDebuff after changes to WoW API
lib.UnitDebuff = function(unit, id)
	for i = 1, 40 do
...
-- wrapper around UnitAura after changes to WoW API
lib.UnitAura = function(unit, id)
	for i = 1, 40 do
...
lib.gen_hpstrings = function(f, unit)
    ...
    hpval.frequentUpdates = 0.2 -- test
    --if f.mystyle == "boss" then name.frequentUpdates = 0.2 end -- test
...
lib.gen_faketarget = function(f)
    ...
    fhp:SetScript('OnEvent', function(self)
        if UnitExists("target") or C_PetBattles.IsInBattle() then
            self:Hide()
        else
            self:Show()
        end
...
local AltPowerPostUpdate = function(app, min, cur, max)
    ...
    local tex, r, g, b, a = GetUnitPowerBarTextureInfo(self.unit, 2)
    if not tex then return end
    if tex == 629029 then
        app:SetStatusBarColor(.7, .3, 1)
    elseif tex == 629031 then
        app:SetStatusBarColor(.1, .8, 1)
    elseif tex == 629033 then
        app:SetStatusBarColor(.5, 1, .2)
    elseif tex == 629035 then
