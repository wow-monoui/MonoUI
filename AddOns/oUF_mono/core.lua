...
local function CreatePlayerStyle(self, unit)
    ...
    if cfg.oUF.settings.ClassBars.enable then
        -- TODO: Mage/Monk bars are missing
        lib.gen_WarlockSpecBar(self)
        lib.gen_Runes(self)
        lib.gen_SoulFragments(self)
        -- TODO: Needs bigger buttons, ala Warlock, but not idea how they are called now.
        lib.gen_HolyPower(self)
        lib.gen_EclipseBar(self)
        lib.gen_TotemBar(self)
        -- Combo points are shared now.
        lib.gen_cp(self)
    end
                    ...
                    for i = 1, 5 do
                        ...
                        if i <= numOpps then
                            ...
                            if class and spec then
                                local color = RAID_CLASS_COLORS[class] --(CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS)[class]
                                if color then
                                    f.Spec:SetFormattedText("|cff%02x%02x%02x%s|r", color.r * 255, color.g * 255, color.b * 255, LOCALIZED_CLASS_NAMES_MALE[class] .. "  -  " .. spec)
                                else
