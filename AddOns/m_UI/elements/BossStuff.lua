...
local function Local_UnitDebuff(unit, spellName)
  local name, icon, _, _, _, etime = UnitDebuff(unit, 1)
...
local function PC_OnEvent(event,...)
  ...
  if pc_event == "SPELL_CAST_START" then
    if pc_spell_cast == "Catalyst: Green" then
		pc_green_casting = 1
		if Local_UnitDebuff("player", "Toxin: Blue") or Local_UnitDebuff("player", "Toxin: Yellow") then pc_announce(gr_t, gr_s) end
    end
    if pc_spell_cast == "Catalyst: Orange" then
		pc_orange_casting = 1
		if Local_UnitDebuff("player", "Toxin: Red") or Local_UnitDebuff("player", "Toxin: Yellow") then  pc_announce(or_t, or_s) end
    end
    if pc_spell_cast == "Catalyst: Purple" then
		pc_purple_casting = 1
		if Local_UnitDebuff("player", "Toxin: Red") or Local_UnitDebuff("player", "Toxin: Blue") then pc_announce(pu_t, pu_s) end
    end
  end
...
local function onUpdate_debuff(self,elapsed)
  ...
  -- get new values
  if Local_UnitDebuff("player", "Toxin: Red") ~= nil    then pc_toxin_red = 1    else pc_toxin_red = 0    end
  if Local_UnitDebuff("player", "Toxin: Blue") ~= nil   then pc_toxin_blue = 1   else pc_toxin_blue = 0   end
  if Local_UnitDebuff("player", "Toxin: Yellow") ~= nil then pc_toxin_yellow = 1 else pc_toxin_yellow = 0 end
...
local function onUpdate_mb(self,elapsed)
  ...
  -- get new value
  local name, icon, _, _, _, etime = UnitDebuff("player", 1)
  if name == "Malicious Blast" then
