...
local backdrop = {
    ...
    tile = false,
    tileSize = 0,
    edgeFile = cfg.nameplates.backdrop_edge,
    edgeSize = 3,
...
local function updateHealth(healthbar, maxHp)
	...
	plate.pct:SetFont(cfg.nameplates.font, cfg.nameplates.fontsize, cfg.nameplates.fontflag)
...
local function NamePlateOnShow(self)
    ...
    local name = self.newPlate.name:GetText() or "Unknown"
    --local level = self.newPlate.level:GetText() or "-1"
    ...
    self._name:SetTextColor(color.r, color.g, color.b)
	self._name:SetFont(cfg.nameplates.font, cfg.nameplates.fontsize, cfg.nameplates.fontflag)
...
local function NamePlateInit(plate)
    ...
    --plate.threat:SetTexture("Interface\\AddOns\\m_Nameplates\\media\\glowTex")
...
local function SearchForNamePlates(self)
	for _, obj in pairs(C_NamePlate.GetNamePlates(true)) do
...
local function RepositionAllNamePlates()
    ...
    for blizzPlate, newPlate in pairs(RNP.nameplates) do
        ...
        if blizzPlate:IsShown() then
            ...
            newPlate:SetParent(blizzPlate) -- Questinable, but at least it sticks on top of them mob
