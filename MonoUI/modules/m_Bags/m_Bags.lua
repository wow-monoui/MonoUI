...
function m_Bags:OnInit()
	...
	f.consumables	= MyContainer:New("Consumables",{Columns = f.main.Settings.Columns, Scale = f.main.Settings.Scale, Bags = "backpack+bags"})
	...
	-- trying to avoid localizing all that shit
	--local LConsumables = select(6, GetItemInfo(6450))
	local LConsumables
	local gl = GetLocale()
	if gl == "deDE" then
		LConsumables = "Verbrauchbar"
	elseif gl == "ruRU" then
		LConsumables = "Расходуемые"
	elseif gl == "zhTW" then
		LConsumables = "消耗品"
	elseif gl == "zhCN" then
		LConsumables = "消耗品"
	elseif gl == "koKR" then
		LConsumables = "소비용품"
	elseif gl == "frFR" then
		LConsumables = "Consommable"
	elseif gl == "itIT" then
		LConsumables = "Consumabili"
	else
		LConsumables = "Consumable"
	end
	...
	if cfg.bags.sets then
		if cfg.bags.consumables then
			onlyBags = function(item) return item.bagID >= 0 and item.bagID <= 4 and not cargBags.itemKeys["setID"](item) and not (item.type and item.type == LConsumables) end
			f.consumables:SetPoint("BOTTOMLEFT", f.sets,"TOPLEFT", 0, 3*f.main.Settings.Scale)
		else
			onlyBags = function(item) return item.bagID >= 0 and item.bagID <= 4 and not cargBags.itemKeys["setID"](item) end
		end
	else
		if cfg.bags.consumables then
			onlyBags = function(item) return item.bagID >= 0 and item.bagID <= 4 and not (item.type and item.type == LConsumables) end
			f.consumables:SetPoint("BOTTOMLEFT", f.main,"TOPLEFT", 0, 3*f.main.Settings.Scale)
		else
	...
	if cfg.bank.sets then
		onlyBank =		function(item) return item.bagID == -1 or item.bagID == -3 or item.bagID >= 5 and item.bagID <= 11 and not cargBags.itemKeys["setID"](item) end
	else
		onlyBank =		function(item) return item.bagID == -1 or item.bagID == -3 or item.bagID >= 5 and item.bagID <= 11  end
	end
	local onlyBagSets =		function(item) return cargBags.itemKeys["setID"](item) and not (item.bagID == -1 or item.bagID == -3 or item.bagID >= 5 and item.bagID <= 11) end
	local onlyBankSets =	function(item) return cargBags.itemKeys["setID"](item) and not (item.bagID >= 0 and item.bagID <= 4) end
	local onlyBagConsumables = function(item) return item.type == LConsumables and not (item.bagID == -1 or item.bagID == -3 or item.bagID >= 5 and item.bagID <= 11) end
...
function MyContainer:OnContentsChanged()
	...
	-- dirty.... but works so whatever
	if m_BagsItemSets:GetHeight()<33 or not cfg.bags.sets			then f.sets:Hide()			else f.sets:Show() end
	if m_BagsBankItemSets:GetHeight()<33 or not cfg.bank.sets		then f.banksets:Hide()		else f.banksets:Show() end
	if m_BagsConsumables:GetHeight()<33 or not cfg.bags.consumables	then f.consumables:Hide()	else f.consumables:Show() end
end
...
function MyContainer:OnCreate(name, settings)
	...
	if not (name == "ItemSets" or name == "BankItemSets" or name == "Consumables") then -- don't need all that junk on "other" sections
...
--[[
 	...
 	elseif name == "Consumables" then
		local consname = self:CreateFontString(nil,"OVERLAY")
		consname:SetPoint("TOPLEFT", self, "TOPLEFT",5,-5)
		consname:SetFont(cfg.bags.general.font, cfg.bags.general.font_size, "THINOUTLINE")
		consname:SetText("Consumables:")
	end
end
