...
-- TODO: Fix 'No set' value when addon is loaded
  ...
  local spc="spec"..GetActiveSpecGroup()
	...
	if set~="" then
        if Broker_EquipmentDB.spec[spc] then
            if C_EquipmentSet.GetEquipmentSetInfoByName(set) then
...
local function menuClick(button, name, icon)
	...
	elseif(IsAltKeyDown()) then
		...
		local setWasEquiped = C_EquipmentSet.UseEquipmentSet(setID)
...
function addon:initialize(level)
	...
	wipe(info)
	for index, setID in pairs(C_EquipmentSet.GetEquipmentSetIDs()) do
		local name, icon, setID, isEquipped, _ = C_EquipmentSet.GetEquipmentSetInfo(setID)
		info.text = string.format('|T%s:20|t %s', icon, name)
		info.arg1 = name
		info.arg2 = icon
		info.func = menuClick
		info.checked = isEquipped or pending and pending == name
		UIDropDownMenu_AddButton(info, level)
	end
...
function addon:UNIT_INVENTORY_CHANGED(event, unit)
	...
	for index, setID in pairs(C_EquipmentSet.GetEquipmentSetIDs()) do
		local name, icon, _, isEquipped, _ = C_EquipmentSet.GetEquipmentSetInfo(setID)
		if(isEquipped) then
