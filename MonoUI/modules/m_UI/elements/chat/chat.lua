...
--	self:SetFont(STANDARD_TEXT_FONT, 12, "THINOUTLINE")
	...
	--Scroll to the bottom button
		...
		local bb = _G[name.."ButtonFrameBottomButtonFlash"]
		if bb then
	...
	-- Remove chat frame channel button
		local bf = _G["ChatFrameChannelButton"]
		bf:Hide()
		bf:SetScript("OnShow",  bf.Hide)
...
---------------- > URL copy Module
--local gsub = gsub
local urlcolor = "77AFFA"
local function SetURL(link)
	link = "|cff"..urlcolor.."|Hurl:"..link.."|h["..link.."]|h|r "
	return link
end
...
for _, event in pairs(cTypes) do
	ChatFrame_AddMessageEventFilter(event, function(self, event, text, ...)
		local newMsg, found = gsub(text, "(%a+)://(%S+)%s?", SetURL("%1://%2"))
		if found > 0 then return false, newMsg, ... end
		newMsg, found = gsub(text, "www%.([_A-Za-z0-9-]+)%.(%S+)%s?", SetURL("www.%1.%2"))
		if found > 0 then return false, newMsg, ... end
		newMsg, found = gsub(text, "([_A-Za-z0-9-%.]+)@([_A-Za-z0-9-]+)(%.+)([_A-Za-z0-9-%.]+)%s?", SetURL("%1@%2%3%4"))
		if found > 0 then return false, newMsg, ... end
		newMsg, found = gsub(text, "(%d+%.%d+%.%d+%.%d+:?%d*)", SetURL("%1"))
		if found > 0 then return false, newMsg, ... end
	end)
end
