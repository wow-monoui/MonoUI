...
function UICoreFrameFlash_OnUpdate(self, elapsed)
  ...
  while FLASHFRAMES[index] do
    ...
    if ((frame._flashTimer > frame._flashDuration) and frame._flashDuration ~= -1) then
      ...
      flashTime =
        flashTime %
        (frame._fadeInTime + frame._fadeOutTime + (frame._flashInHoldTime or 0) + (frame._flashOutHoldTime or 0))
...
do
  ...
  hooksecurefunc(
    "UIFrameFlash",
    function(frame, fadeInTime, fadeOutTime, flashDuration, showWhenDone, flashInHoldTime, flashOutHoldTime)
      if (frame) then
        if not issecurevariable(frame, "fadeInTime") or not issecurevariable(frame, "flashTimer") then
